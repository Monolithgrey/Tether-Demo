...
/* Methodology disclosure — builds trust at $40K */
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:bd}}>
      ...
      <div style={{maxWidth:740,margin:"0 auto",padding:"48px 40px 32px"}}>
        ...
        {step===0&&<Fade show={r}>
          ...
          {/* Compact org bar */}
          <Card shadow={1} mb={24} pad="20px 24px">
            <div style={{display:"flex",alignItems:"center",gap:24,flexWrap:"wrap"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:10,fontWeight:700,color:C.kf,letterSpacing:".06em"}}>SPECIALTY</span>
                <select value={spec} onChange={e=>setSpec(e.target.value)}
                  style={{padding:"6px 12px",borderRadius:4,border:`1px solid ${C.b}`,background:C.sf,fontSize:12,fontFamily:bd,color:C.k,cursor:"pointer"}}>
                  {Object.entries(SPECS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
                </select>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:10,fontWeight:700,color:C.kf,letterSpacing:".06em"}}>PROVIDERS</span>
                <span style={{fontSize:16,fontWeight:600,color:C.k,fontFamily:mn}}>{bProv}</span>
                <input type="range" min={5} max={500} step={5} value={bProv} onChange={e=>setBProv(+e.target.value)}
                  style={{width:80,cursor:"pointer",opacity:.6}}/>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:10,fontWeight:700,color:C.kf,letterSpacing:".06em"}}>MONTHS LIVE</span>
                <span style={{fontSize:16,fontWeight:600,color:C.k,fontFamily:mn}}>{monthsLive}</span>
                <input type="range" min={1} max={36} step={1} value={monthsLive} onChange={e=>setMonthsLive(+e.target.value)}
                  style={{width:60,cursor:"pointer",opacity:.6}}/>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:10,fontWeight:700,color:C.kf,letterSpacing:".06em"}}>REV/VISIT</span>
                <span style={{fontSize:16,fontWeight:600,color:C.k,fontFamily:mn}}>{fmt(rpv)}</span>
                <input type="range" min={100} max={800} step={5} value={rpv} onChange={e=>setBRevPerVisit(+e.target.value)}
                  style={{width:60,cursor:"pointer",opacity:.6}}/>
              </div>
            </div>
          </Card>
                ...
                {metrics.map(m=>(
                  <div key={m.label+"b"} style={{marginBottom:24}}>
                    ...
                    <div style={{position:"relative",height:24,display:"flex",alignItems:"center"}}>
                      <div style={{position:"absolute",left:0,right:0,height:1,background:C.bl}}/>
                      <div style={{position:"absolute",left:0,width:`${((m.b-m.minB)/(m.maxB-m.minB))*100}%`,height:1,background:`${C.r}60`}}/>
                      ...
                      <div style={{position:"absolute",left:`${((m.b-m.minB)/(m.maxB-m.minB))*100}%`,transform:"translateX(-50%)",
                        width:10,height:10,borderRadius:"50%",background:C.sf,border:`2px solid ${C.r}`,boxShadow:sh1,pointerEvents:"none",zIndex:1}}/>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{padding:"28px 28px",background:C.sh}}>
                ...
                {metrics.map(m=>{
                  ...
                  return <div key={m.label+"a"} style={{marginBottom:24}}>
                    ...
                    <div style={{position:"relative",height:24,display:"flex",alignItems:"center"}}>
                      <div style={{position:"absolute",left:0,right:0,height:1,background:C.bl}}/>
                      <div style={{position:"absolute",left:0,width:`${((m.a-m.minA)/(m.maxA-m.minA))*100}%`,height:1,background:C.t}}/>
                      ...
                      <div style={{position:"absolute",left:`${((m.a-m.minA)/(m.maxA-m.minA))*100}%`,transform:"translateX(-50%)",
                        width:10,height:10,borderRadius:"50%",background:C.sf,border:`2px solid ${C.t}`,boxShadow:`0 2px 8px ${C.t}40`,pointerEvents:"none",zIndex:1}}/>
                    </div>
                  </div>;
                })}
              </div>
  ...
  return <Fade delay={i*80} show={true}>
    <div onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{padding:"28px 28px",cursor:"pointer",background:h?C.sh:C.sf,border:`1px solid ${h?C.bd:C.b}`,
        borderRadius:6,marginBottom:12,transition:"all .15s",position:"relative",boxShadow:h?sh2:sh1}}>
...
function CaseStudyPath({onHome}){
  ...
  useEffect(()=>{setR(false);setTimeout(()=>setR(true),40)},[sel]);
